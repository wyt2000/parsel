...
def compute_area(whs):
    widths_sum = sum(pair[0] for pair in whs)
    heights_max = max(pair[1] for pair in whs)
    return widths_sum * heights_max
...
def minimum_area(whs):
    half_length = len(whs) // 2
    subsets = enumerate_subsets_at_most_k(list(range(len(whs))), half_length)
    min_area = float('inf')
